...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "dhcp" do |serverd|
    serverd.vm.box = "generic/ubuntu1804"
    serverd.vm.hostname = "dhcpServer"
    serverd.vm.network  "private_network", ip: "192.168.56.2"
    serverd.vm.provider "virtualbox" do |vb|
    vb.customize ["modifyvm", :id, "--memory", "512", "--cpus", "1", "--name", "dhcpServer"]
   end
   serverd.vm.provision "shell", privileged:true, inline: $script
  end
